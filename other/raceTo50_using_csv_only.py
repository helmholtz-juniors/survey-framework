...
FRAMEWORK_PATH = "/home/jenpopp/Documents/HeJus/N2survey/survey-framework"
# path to the survey data 
INPUT_PATH = "/home/jenpopp/Documents/HeJus/N2survey/data/survey_2024"
# path where the plot is supposed to be saved
OUTPUT_PATH = "/home/jenpopp/Documents/HeJus/N2survey/Figures_2024"
...
survey_files = get_survey_csv_files(INPUT_PATH)
...
for i, CSV_FILE_NAME in enumerate(sorted_survey_files):
    ...
    # Get the survey data 
    csv_file = pd.read_csv(INPUT_PATH + '/' + CSV_FILE_NAME)
    ...
    df_centers[f"percentages_week{i+1}"] = df_centers[f"counts_week{i+1}"]/df_centers['N_drs']*100
...
#make a nice figure 
fig,ax = plt.subplots(1,1,figsize=(6.2,6.2*(11/16)), dpi=300)
...
for j in range(1,len(df_centers_sorted[percentage_columns].columns)):
    df_diff.iloc[:,j]=df_centers_sorted[percentage_columns].iloc[:,j]-df_centers_sorted[percentage_columns].iloc[:,j-1]

# Get the colors for the bar plot 
#The survey is running for 6 weeks  
blues = get_blues(N_weeks)[0:len(survey_files)]

# plot the bars 
bars=df_diff.plot.barh(stacked=True,zorder=3,legend=False,color=blues, ax=ax)
...
ax.text(0.5, 1.05, title_first_line, ha='center', va='bottom', fontsize=14, fontweight='bold', transform=ax.transAxes)
ax.text(0.5, 1.01, title_second_line, ha='center', va='bottom', fontsize=11, transform=ax.transAxes)

#####Adding a legend 
legend_labels = [f"Week {week+1}" for week in range(len(survey_files))]
patches = [mpl.patches.Patch(color=color, label=label) for color, label in zip(blues, legend_labels)]
