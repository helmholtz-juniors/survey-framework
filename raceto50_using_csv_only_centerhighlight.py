...
from plotting.helmholtzcolors import get_blues, get_greens, helmholtzgreen, helmholtz_information
...
for _center_offset, _center in enumerate(df_diff.index):
    # make a nice figure
    ...
    edges = ['None' for _ in range(len(df_diff))]
    edges = helmholtz_information

    # plot the bars
    bars = df_diff.plot.barh(stacked=True, zorder=3, legend=False, color=blues, edgecolor='none', ax=ax)  # type: ignore[call-overload]
    # Highlight center bar
    ax.barh(_center_offset, sum(df_diff.loc[_center]), left=0, color='none', edgecolor=edges, linewidth=5, height=0.6)
    ...
    # crown_path, attributes = svg2paths('../figures_2024/longest_bar.svg')
    # crown_marker = parse_path(attributes[0]['d'])
    # crown_marker.vertices -= crown_marker.vertices.mean(axis=0)

    # # crown_marker = crown_marker.transformed(mpl.transforms.Affine2D().rotate_deg(120))
    ...
    #     markersize=50,
    #     # zorder=4,
    #     color=helmholtzgreen
    # )

    plt.savefig(OUTPUT_PATH / f"RaceTo50_{formatted_date}_{_center}.png")
