...
def _parse_question_description(question: Tag) -> str:
    ...
    if len(directive_sections) >= 1:
        directive: Tag = directive_sections[0]
        description = " ".join(
            [_get_clean_string(text) for text in directive.find_all("text")]
        )
        if len(directive_sections) > 1:
            warn(
...
def _parse_single_question_response(
    ...
    # Parse fixed question (i.e. with choises)
        ...
        if contingent_question is not None:
            contingent_question = contingent_question
...
def _parse_question(
    ...
    subquestions = _parse_question_sub_questions(question)
    ...
    if subquestions_count:
        ...
        assert contingent is None, (
            f"Unexpected question type for question {question}."
            " Sub question with contingent questions are not supported."
        )
