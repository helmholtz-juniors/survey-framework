...
def rate_mental_health(
    ...
    if condition == "state_anxiety":
        ...
        raise ValueError(
            ...
            "Please consult your friendly local psychiatrist."
        )
    ...
    invert_dict = {
        the_class: code for code, the_class in zip(choice_codes, classes, strict=False)
    }
    ...
    for column, conv in zip(responses.columns, conversion, strict=False):
        df[f"{column}_score"] = (
            responses[column]
            .map(choices)
            .map(conversion_dicts[conv], na_action="ignore")
        )
